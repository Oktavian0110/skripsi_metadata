...
class MetadataApp:
        ...
        try:
            ...
            if os.path.exists(csv_path):
                if messagebox.askyesno("Konfirmasi", f"Anda yakin ingin menghapus semua data di {file_name}?"):
                    os.remove(csv_path); messagebox.showinfo("Sukses", f"Semua data di {file_name} telah dihapus."); refresh_function()
            else: messagebox.showinfo("Info", "File data tidak ditemukan (sudah bersih).")
        except Exception as e: messagebox.showerror("Error", f"Gagal menghapus file: {e}")

    def load_and_display_pdf_data(self):
        # --- PERBAIKAN: Menyimpan indeks DataFrame sebagai ID unik di tabel ---
        ...
        if not os.path.exists(csv_path):
            ...
            return
        try:
            full_df = pd.read_csv(csv_path).reset_index(drop=True) # Reset index agar urut
            ...
            # Gunakan 'index' dari DataFrame sebagai ID unik (iid) untuk setiap baris
    ...
    def load_and_display_git_data(self):
        ...
        try:
            full_df = pd.read_csv(csv_path).reset_index(drop=True) # Reset index agar urut
            cols = ("Commit Hash", "Pesan", "Author", "Tanggal")
            tree = ttk.Treeview(self.git_results_frame, columns=cols, show="headings")
            for col in cols:
                tree.heading(col, text=col)
                if col == "Commit Hash": tree.column(col, width=100)
                elif col == "Pesan": tree.column(col, width=300)
                else: tree.column(col, width=120)
            
            # Gunakan 'index' dari DataFrame sebagai ID unik (iid) untuk setiap baris
            for index, row in full_df.iterrows():
                short_hash = str(row["commit_hash"])[:7]
                tree.insert("", "end", iid=index, values=(short_hash, row["pesan"], row["author"], row["tanggal"]))
            tree.pack(fill="both", expand=True)
    ...
    def delete_selected_pdf_row(self):
        ...
        # Ambil indeks unik dari baris yang dipilih
        index_to_delete = int(selected_items[0])
        
        if messagebox.askyesno("Konfirmasi Hapus", f"Anda yakin ingin menghapus baris data yang dipilih?"):
            try:
                csv_path = os.path.join("output", "pdf_metadata.csv")
                df = pd.read_csv(csv_path)
                # Langsung hapus baris berdasarkan indeks uniknya
    ...
    # Fungsi-fungsi Visualisasi (Tidak ada perubahan)
    def setup_viz_tab(self):
        # ... (isi fungsi ini tetap sama) ...
        button_frame = ttk.Frame(self.tab_viz); button_frame.pack(side="bottom", fill="x", pady=5)
        ttk.Button(button_frame, text="Grafik Commit (Git)", command=self.show_commit_chart).pack(side="left", padx=10, pady=5, expand=True)
        ttk.Button(button_frame, text="Grafik Dokumen (PDF)", command=self.show_pdf_chart).pack(side="right", padx=10, pady=5, expand=True)
    ...
    def show_commit_chart(self):
        ...
        try:
            ...
            commit_counts.plot(kind='bar', ax=ax, color='skyblue')
            ax.set_title("Jumlah Commit per Author"); ax.set_ylabel("Jumlah Commit"); ax.set_xlabel("Author")
